...
(function () {
  ...
  var VALID_RESPONSES = { "OK": 1, "BUSY": 1, "TOO_LARGE": 1, "INVALID_PCM": 1, "PTT_BIN_MISSING": 1, "AUDIO_DEVICE_BUSY": 1 };
  ...
  function stopRecording(fromMaxDuration) {
    ...
    uploadXhr.onload = function () {
      ...
      } else if (payload === "AUDIO_DEVICE_BUSY") {
        flashAndReset("Audio busy", "ptt-error", 1800);
    ...
    var pendingUpload = uploadXhr;
    window.ensureCsrfToken().then(function(token) {
      if (uploadXhr !== pendingUpload) return;
      pendingUpload.setRequestHeader("X-CSRF-Token", token);
      pendingUpload.send(blob);
    }).catch(function() {
      if (uploadXhr === pendingUpload) {
        uploadXhr = null;
        pendingUpload.abort();
        flashAndReset("Reload page", "ptt-error", 1800);
      }
    });
  }
  ...
  function initPttAudio() {
    ...
    pttStatusXhr.onload = function () {
      // action.cgi appends an HTML footer (<hr/>) to every non-JSON response,
      // so the body is "OK\n<hr/>" — a strict comparison against "OK" branded
      // a healthy backend as missing and greyed out every PTT control. Only
      // the first line carries the status.
      var status = (pttStatusXhr.responseText || "").split("\n")[0].trim();
      if (status && status !== "OK") {
        setPttUnavailable("PTT unavailable", "Camera audio output backend is missing.");
        return;
      }
      if (pttVol) { pttVol.disabled = false; pttVol.title = ""; }
      if (pttTestBtn) pttTestBtn.disabled = false;
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initPttAudio, { once: true });
  } else {
    initPttAudio();
  }
