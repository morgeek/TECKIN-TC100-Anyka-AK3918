    ...
    initDashboard: function() {
        ...
        fetch('cgi-bin/state.cgi?cmd=fullconfig&_=' + Date.now())
            ...
            .then(function(res) {
                ...
                // 1. Dashboard Tab Population
                if (byId('dash_ip')) byId('dash_ip').textContent = res.system.hostname || '--.--.--.--';
                if (byId('dash_rtsp_link_0')) {
                    var ip = window.location.hostname;
                    byId('dash_rtsp_link_0').href = 'rtsp://' + ip + ':' + res.video.rtsp_port + '/video0_unicast';
                }

                // 2. Video Tab Population
                if (byId('video_size0')) byId('video_size0').value = res.video.main.width + 'x' + res.video.main.height;
                if (byId('fps0')) byId('fps0').value = res.video.main.fps;
                if (byId('brbitrate0')) byId('brbitrate0').value = res.video.main.bitrate;

                // 3. Audio Tab Population
                if (byId('samplerate')) byId('samplerate').value = res.audio.samplerate;

                // 4. ISP/OSD Tab Population
                if (byId('osdenabled')) {
                    byId('osdenabled').checked = !!res.osd.enabled;
                    byId('osdenabled').value = res.osd.enabled ? '1' : '0';
                }
                if (byId('osdtext')) byId('osdtext').value = res.osd.text;

                // 5. MQTT Tab Population
                if (byId('mqtt_enabled')) byId('mqtt_enabled').checked = !!res.mqtt.enabled;
                if (byId('mqtt_host')) byId('mqtt_host').value = res.mqtt.host;

                // Set up System Buttons
                var btnReboot = byId('btnReboot');
                if (btnReboot) {
                    btnReboot.onclick = function() {
                        if (!confirm('Reboot camera?')) return;
                        fetch('cgi-bin/action.cgi?cmd=reboot').then(function() {
                            showResult('Rebooting... please wait 60s', 'is-success');
                        });
                    };
                }
                ...
                if (btnFreeRam) {
                    btnFreeRam.onclick = function() {
                        fetch('cgi-bin/action.cgi?cmd=clear_mem').then(function() {
                            showResult('RAM cleared', 'is-success');
                        });
                    };
                }
            })
        ...
        // Also fetch health snapshot for the real-time gauges every 3s
        var pollHealth = function() {
            if (!window.isHostStillActive(dash)) return;
            fetch('cgi-bin/state.cgi?cmd=healthsnapshot&_=' + Date.now())
                .then(function(r) { return r.json(); })
                .then(function(res) {
                    if (!res) return;
