(function () {
  ...
  window.EliteUI = {
    initDashboard: function() {
        var dash = byId('settings_dashboard');
        if (!dash) return;
        ...
        fetch('cgi-bin/state.cgi?cmd=healthsnapshot&_=' + Date.now())
            ...
            .then(function(res) {
                ...
                if (cpuBar) {
                    cpuBar.style.width = res.cpu + '%';
                    cpuBar.className = 'ui-progress-bar ' + (res.cpu > 80 ? 'is-danger' : (res.cpu > 40 ? 'is-warning' : 'is-success'));
                }
                ...
                if (memBar) {
                    memBar.style.width = res.ram_percent + '%';
                    memBar.className = 'ui-progress-bar ' + (res.ram_percent > 90 ? 'is-danger' : (res.ram_percent > 60 ? 'is-warning' : 'is-success'));
                }

                if (byId('dash_ip')) byId('dash_ip').textContent = res.primary_ip || '--.--.--.--';
                if (byId('dash_gw')) byId('dash_gw').textContent = res.default_gateway || 'n/a';
                ...
                // Performance Profile
                var perfSel = byId('performance_profile');
                if (perfSel) perfSel.value = res.perfprofile || 'balanced';
            })
