    ...
    initDashboard: function() {
        ...
        fetch('cgi-bin/state.cgi?cmd=fullconfig&_=' + Date.now())
            ...
            .then(function(res) {
                ...
                if (byId('dash_gw')) byId('dash_gw').textContent = res.system.gateway || 'n/a';
                if (byId('dash_rtsp_link_0')) {
                    byId('dash_rtsp_link_0').href = 'rtsp://' + window.location.hostname + ':' + res.video.rtsp_port + '/video0_unicast';
                }

                // 2. Video & Audio
                setVal('video_size0', res.video.main.width + 'x' + res.video.main.height);
                setVal('video_codec0', res.video.main.codec);
                setVal('fps0', res.video.main.fps);
                setVal('brbitrate0', res.video.main.bitrate);
                setVal('goplen0', res.video.main.gop);
                setVal('video_format0', res.video.main.format);
                setVal('minqp0', res.video.main.minqp);
                setVal('maxqp0', res.video.main.maxqp);
                setVal('smartmode0', res.video.main.smartmode);

                setVal('video_size1', res.video.sub.width + 'x' + res.video.sub.height);
                setVal('fps1', res.video.sub.fps);
                setVal('brbitrate1', res.video.sub.bitrate);

                setVal('samplerate', res.audio.samplerate);
                setVal('audioCodec0', res.audio.codec_main);
                setVal('audio_vol', res.audio.volume);

                // 3. ISP & OSD
                setVal('daynightlum', res.isp.daynight_lum);
                setVal('daynightawb', res.isp.daynight_awb);
                setVal('nightdaylum', res.isp.nightday_lum);
                setVal('nightdayawb', res.isp.nightday_awb);

                setVal('osdenabled', res.osd.enabled);
                setVal('osdtext', res.osd.text);
                setVal('frontcolor', res.osd.frontcolor);
                setVal('backcolor', res.osd.backcolor);
                setVal('osdalpha', res.osd.alpha);
                setVal('osdfontsize0', res.osd.fontsize0);
                setVal('osdx0', res.osd.x0);
                setVal('osdy0', res.osd.y0);
                setVal('imageFlip', res.video.flip);

                // 4. Automation
                setVal('mqtt_enabled', res.mqtt.enabled);
                setVal('mqtt_host', res.mqtt.host);
                setVal('mqtt_port', res.mqtt.port);
                setVal('mqtt_user', res.mqtt.user);
                setVal('mqtt_topic_root', res.mqtt.topic_root);
                setVal('mqtt_discovery', res.mqtt.discovery);

                setVal('mdsens', res.services.motion_sens);
                setVal('sound_det_threshold', res.services.sound_det_threshold);
                setVal('motion_led', res.services.motion_led);

                setVal('motion_activated', res.recording.motion_activated);
                setVal('postrec', res.recording.postrec);
                setVal('maxduration', res.recording.maxduration);
                setVal('reserved_mb', res.recording.reserved_mb);

                // 5. Network
                setVal('hostname', res.system.hostname);
                setVal('rtsp_port', res.video.rtsp_port);
                setVal('telnet_port', res.services.telnet_port);
                setVal('timezone', res.system.timezone);
                setVal('ntp_server', res.system.ntp_server);

                // 6. System & Tuning
                setVal('lightweight_mode', res.boot.lightweight_mode);
                setVal('security_hardening', res.boot.security_hardening);
                setVal('mem_guard_enable', res.system.mem_guard.enable);
                setVal('mem_guard_warn_kb', res.system.mem_guard.warn_kb);
                setVal('mem_guard_crit_kb', res.system.mem_guard.crit_kb);
                setVal('mem_guard_interval', res.system.mem_guard.interval);

                setVal('reboot_enable', res.system.reboot_schedule.enable);
                setVal('reboot_schedule_hour', res.system.reboot_schedule.hour);
                setVal('reboot_schedule_min', res.system.reboot_schedule.min);
                setVal('reboot_schedule_dow', res.system.reboot_schedule.dow);
                ...
                // 8. Telegram & Syslog
                setVal('telegram_enable', res.telegram.enabled);
                setVal('telegram_token', res.telegram.token);
                setVal('telegram_chat_id', res.telegram.chat_id);
                setVal('syslog_enable', res.syslog.enabled);
                setVal('syslog_host', res.syslog.host);
                setVal('syslog_port', res.syslog.port);

                // 9. Peripherals
                setVal('led_front', res.peripherals.led_front);
                setVal('led_red', res.peripherals.led_red);
                ...
                dash.querySelectorAll('form').forEach(function(f) {
                    f.onsubmit = function(ev) {
                        ...
                        csrfFetch(f.action, {
                            ...
                            if (r.ok) showResult('Settings applied successfully', 'is-success');
                            else showResult('Failed to apply settings', 'is-danger');
                        });
                    };
                });
