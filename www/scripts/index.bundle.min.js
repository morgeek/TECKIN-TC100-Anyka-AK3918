(function () {
  ...
  function updateSystemLoadState(sysusageText, cpuPercentHint, ramPercentHint) {
    ...
    if (cameraState && cameraDot) {
      ...
      cameraState.textContent = pressure > 80 ? "Camera under heavy load" : "Camera operational";
  ...
  function ensureCsrfToken() {
    ...
    if (!csrfTokenRequest) {
      csrfTokenRequest = fetch("cgi-bin/state.cgi?cmd=statusline", { cache: "no-store" })
        .then(function(r) { if (!r.ok) throw new Error("Cannot read CSRF token"); return r.json(); })
        .then(function(data) {
          if (!data.csrf_token) throw new Error("Missing CSRF token");
