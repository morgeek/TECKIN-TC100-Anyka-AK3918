...
### Notes
- The persistent listener trades resident memory for the loss of connect/teardown churn every ~13 s. Measured on both cameras after the rollout: RAM went from ~14.8 MB to **17.3 MB (IPCAMERA1) / 19.5 MB (IPCAMERA2)** of 33.4 MB, CPU unchanged at **43 % / 51 %** against a 41–43 % baseline. On a 33 MB device that memory cost is a real trade, not a free win — set `MQTT_PERSISTENT_SUBSCRIBE=0` if memory matters more than command latency on a given unit.
- Beware when measuring: `state.cgi?cmd=sysusage` read 98–100 % CPU immediately after a burst of HTTPS polls, because every CGI request is itself a fork on this core. Let the device idle ~90 s before trusting a CPU figure; `/proc/loadavg` (~1.2 steady, dominated by the RTSP stream) is the more honest signal.
